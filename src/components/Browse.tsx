  ...
  return (
    <div className="min-h-screen bg-[#fafaf8] p-10 text-black">
      ...
      <input
        ...
        className="w-full max-w-md border border-neutral-300 bg-neutral-100 rounded px-3 py-2 mb-8 outline-0"
      />
      {filtered.length === 0? (
        ...
        <table className="w-full">
            ...
            <tbody>
                {filtered.map((track, index)=>(
                    <tr className="border-b border-neutral-200" key={track.id}>
                        <td className="py-3 px-2 font-mono text-neutral-500">{String(index+1).padStart(2, "0")}</td>
                        <td className="py-3">
                            <p className="font-medium">{track.title}</p>
                            <p className="text-xs text-neutral-500">{track.artist}</p>
                        </td>
                        <td className="py-3 text-right font-mono text-neutral-600">{track.duration}</td>
                    </tr>
                ))}
            </tbody>
        </table>
